...
1. **Create the App** (once per account/org):
   ...
   *Settings → Developer settings → GitHub Apps → New*.
   ...
   - **Repository permissions:**
     ...
     - **Administration: Read and write** — only if you want the App to also
       manage rulesets; not required just to bypass one.
...
unprotected fresh repo and a protected one — no re-generation needed. The
container image push in `release.yml` is separate and controlled by the
`enable_docker_publishing` answer.
